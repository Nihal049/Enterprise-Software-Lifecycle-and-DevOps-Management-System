...
import { Lock, Mail, Loader2, ShieldCheck, ArrowRight, Sparkles, CheckCircle2, KeyRound } from 'lucide-react';
...
export default function Login() {
  ...
  // RESTORED: Quick-fill helper for developer testing
  const fillDemoCredentials = (role) => {
    if (role === 'devops') {
      setEmail('devops_admin@neuroforge.com');
      setPassword('admin123');
    } else {
      setEmail('developer@neuroforge.com');
      setPassword('dev123');
    }
  };
      ...
      <div className="relative z-10 w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="p-7 sm:p-10 lg:p-12 flex items-center">
          <div className="w-full max-w-md mx-auto">
            ...
            {/* RESTORED: Quick-Fill Demo Buttons */}
            <div className="mt-8 pt-5 border-t border-slate-100 text-center">
              <p className="text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-3">Quick Fill Demo Accounts</p>
              <div className="flex gap-2">
                <button 
                  type="button" 
                  onClick={() => fillDemoCredentials('devops')}
                  className="flex-1 py-2 px-3 bg-slate-50 hover:bg-slate-100 text-slate-600 text-xs font-semibold rounded-lg border border-slate-200 transition-colors flex items-center justify-center gap-1.5"
                >
                  <KeyRound size={12} className="text-blue-500" /> DevOps Admin
                </button>
                <button 
                  type="button" 
                  ...
                  className="flex-1 py-2 px-3 bg-slate-50 hover:bg-slate-100 text-slate-600 text-xs font-semibold rounded-lg border border-slate-200 transition-colors flex items-center justify-center gap-1.5"
                >
                  <KeyRound size={12} className="text-indigo-500" /> Developer
                </button>
              </div>
            </div>
